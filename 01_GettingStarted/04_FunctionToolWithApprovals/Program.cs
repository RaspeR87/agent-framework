...
AIAgent agent = new AzureOpenAIClient(
    ...
    .CreateAIAgent(instructions: "You are a funny assistant who loves dogs.", tools: [new ApprovalRequiredAIFunction(AIFunctionFactory.Create(TalkAboutDogs))]);

// Non-streaming agent interaction with function tools.
AgentThread thread = agent.GetNewThread();
var response = await agent.RunAsync("Tell me something about Huskies", thread);
var userInputRequests = response.UserInputRequests.ToList();

// For streaming use:
// var updates = await agent.RunStreamingAsync("Tell me something about Huskies", thread).ToListAsync();
